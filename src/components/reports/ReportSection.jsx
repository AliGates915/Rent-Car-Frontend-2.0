...
export default function ReportSection({ config, reportType }) {
  ...
  if (!config || !config.reportTypes) {
    console.error('ReportSection: Invalid config or missing reportTypes', config);
    return (
      <div className="text-center text-red-600 py-12">
        Error: Report configuration is missing. Please check module configuration.
      </div>
    );
  }

  const reportConfig = config.reportTypes[reportType];
  
  if (!reportConfig) {
    return (
      <div className="text-center text-yellow-600 py-12">
        Report type "{reportType}" is not configured.
      </div>
    );
  }
  ...
  useEffect(() => {
    fetchReport();
  }, [reportType, dateRange.startDate, dateRange.endDate, selectedDate]);
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">
              {config.title} - {reportType.replace('-', ' ').toUpperCase()}
            </h2>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-2 px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition disabled:opacity-50"
            >
...
function DaybookReport({ data }) {
  ...
  const total_debit = data?.total_debit || 0;
  const total_credit = data?.total_credit || 0;
  ...
  if (!entries || entries.length === 0) {
    return (
      <div className="text-center text-gray-500 py-12">
        No daybook entries found for this date
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="bg-gray-50 rounded-lg p-4">
        <div className="grid grid-cols-3 gap-4">
          <div><p className="text-sm text-gray-600">Date</p><p className="text-lg font-semibold">{date}</p></div>
          <div><p className="text-sm text-gray-600">Total Debit</p><p className="text-lg font-semibold text-blue-600">Rs.{total_debit.toLocaleString()}</p></div>
          <div><p className="text-sm text-gray-600">Total Credit</p><p className="text-lg font-semibold text-orange-600">Rs.{total_credit.toLocaleString()}</p></div>
        </div>
        <div className="mt-2 pt-2 border-t">
          <p className="text-sm text-gray-600">Net Balance</p>
          <p className={`text-lg font-semibold ${net >= 0 ? 'text-green-600' : 'text-red-600'}`}>Rs.{net.toLocaleString()}</p>
        </div>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-2 text-right">Debit (Rs.)</th>
              <th className="px-4 py-2 text-right">Credit (Rs.)</th>
              ...
              <tr key={idx} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-2">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    entry.entry_type === 'payment' || entry.entry_type === 'receipt' 
                      ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                  }`}>{entry.entry_type}</span>
                </td>
                <td className="px-4 py-2">{entry.description || '-'}</td>
                <td className="px-4 py-2 text-right">Rs.{Number(entry.debit || 0).toLocaleString()}</td>
                <td className="px-4 py-2 text-right">Rs.{Number(entry.credit || 0).toLocaleString()}</td>
                <td className="px-4 py-2 text-sm">{format(new Date(entry.created_at), 'dd MMM yyyy HH:mm')}</td>
              </tr>
            ))}
          </tbody>
          <tfoot className="bg-gray-50 font-semibold">
            <tr>
              ...
              <td className="px-4 py-2 text-right">Rs.{total_debit.toLocaleString()}</td>
              <td className="px-4 py-2 text-right">Rs.{total_credit.toLocaleString()}</td>
              <td></td>
            </tr>
          </tfoot>
        </table>
